...
from utils.dataset_utils_fixed import check, separate_data, split_data, save_file
...
def generate_cifar100_with_clusters(data_dir, client_count, class_count, is_niid, is_balanced, partition_type, num_concepts=5, iterations=200, num_drifts=5):
    ...
    for iteration in range(iterations):
        ...
        # 跟踪复制的样本，为每个客户端维护一个字典
        # 格式: {client_id: {复制的样本索引: [原始样本索引]}}
        copied_samples_tracking = {}
        
        # 根据每个客户端当前的概念调整数据分布
        for client_id in range(client_count):
            copied_samples_tracking[client_id] = {}
            ...
            for cls in range(class_count):
                ...
                if current_count > target_count:
                    ...
                    if addition > 0 and current_count > 0:
                        ...
                        current_idx = len(client_x)
                        
                        for _ in range(addition):
                            # 随机选择一个原始样本索引
                            orig_idx = np.random.choice(cls_indices)
                            
                            # 使用数据增强而不是直接复制
                            augmented_img = augment_image(client_x[orig_idx])
                            new_x.append(augmented_img)
                            new_y.append(cls)
                            
                            # 记录这个新样本与原始样本的关系
                            if current_idx not in original_indices_map:
                                original_indices_map[current_idx] = []
                            original_indices_map[current_idx].append(orig_idx)
                            current_idx += 1
                        
                        # 记录复制的样本信息
                        copied_samples_tracking[client_id].update(original_indices_map)
                        
                        # 添加新样本
                        if new_x:
                            client_x = np.concatenate([client_x, np.array(new_x)])
                            client_y = np.concatenate([client_y, np.array(new_y)])
        ...
        # 划分训练和测试数据，确保测试集不包含训练集的样本副本
        train_data, test_data = split_data(X, y, copied_samples_tracking)
