...
def parse_args():
    parser = argparse.ArgumentParser(description='运行FedDrift算法')
    ...
    parser.add_argument('--detection_threshold', type=float, default=0.1,
                      help='概念漂移检测阈值')
    parser.add_argument('--visualize_clusters', action='store_true', default=True,
                      help='是否可视化集群')
...
if __name__ == '__main__':
    ...
    # 添加FedDrift特有参数
    cmd += f" --detection_threshold {args.detection_threshold}"
    
    # 添加布尔参数
    if args.visualize_clusters:
        cmd += " --visualize_clusters"
    if args.verbose:
        cmd += " --verbose"
